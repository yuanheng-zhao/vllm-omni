...
def split_text_into_sentences(text: str) -> list[str]:
    """Sentence split for streaming thinker -> talker text.

    This is NOT a faithful port of Ming's streaming segmenter
    detect_sentence_boundaries; it intentionally diverges:
    - It is *append-only stable* — re-running on a longer prefix of the same
        text never changes an already-completed (non-trailing) segment,
    - It does NOT pack fragments to`max_length or apply Ming's newline/CJK-comma rewriting.
    """

    # TODO: evaluate the splitting on test datasets
    if not text:
        return []
    ...
    def _min_tokens() -> int:
        # Mirror detect_sentence_boundaries: larger gate before the first flush.
        return 12 if not sentences else 8

    def _ready_to_flush() -> bool:
        return len(tokenize_mixed_text("".join(buf))) >= _min_tokens()
    ...
    for i, ch in enumerate(text):
        if ch == "\n":
            # Soft boundary: only break if enough has accumulated, otherwise
            # absorb (the newline itself is dropped) and keep going.
            if buf and _ready_to_flush():
                _flush()
            continue
        ...
        if ch in "。！？!?":
            ...
            # Decimal like 3.14 -> not a sentence boundary.
